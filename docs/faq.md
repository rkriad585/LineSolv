...
As you type, LineSolv shows suggestions for variables, functions, and constants. Autocomplete appears when you start typing a word. You can navigate suggestions with `Arrow Up`/`Arrow Down` and accept with `Enter` or `Tab`. Autocomplete can be toggled in **Settings > General** or via `config.toml` under `[settings] -> autocomplete_enabled` (`"true"` or `"false"`).
...
Yes. Drag and drop notes in the sidebar to reorder them. You can also drag and drop folders to reorganize your note structure. The order is persisted in the database. Drag and drop can be toggled in **Settings > General** or via `config.toml` under `[settings] -> drag_and_drop` (`"true"` or `"false"`).
...
LineSolv comes with 27 built-in themes:
...
| **Blue Trust Dark**         | Deep navy with calming blue accents     |
| **Blue Trust Light**        | Clean white with calming blue accents   |
| **Orange Energy Dark**      | Dark with vibrant orange accents        |
| **Orange Energy Light**     | Light with warm orange accents          |
| **Green Growth Dark**       | Dark with natural green accents         |
| **Green Growth Light**      | Light with natural green accents        |
| **Yellow Optimism Dark**    | Dark with bright yellow accents         |
| **Yellow Optimism Light**   | Light with sunny yellow accents         |
| **Purple Innovation Dark**  | Dark with vibrant purple accents        |
| **Purple Innovation Light** | Light with creative purple accents      |
| **Red Passion Dark**        | Dark with bold red accents              |
| **Red Passion Light**       | Light with bold red accents             |

The psychology themes (Blue Trust, Orange Energy, Green Growth, Yellow Optimism, Purple Innovation, Red Passion) are designed to evoke specific emotional responses through color.
...
Open **Settings > General** and use the **Result Panel** toggle under the _Behavior_ section. When off, the right-side results column is hidden and only input lines are shown. You can also toggle it via `config.toml` under `[settings] -> result_panel_enabled` (`"true"` or `"false"`).

### How do I toggle line wrapping?

Open **Settings > General** and use the **Line Wrap** toggle under the _Behavior_ section. When off, long lines extend horizontally instead of wrapping. You can also toggle it via `config.toml` under `[settings] -> line_wrap_enabled` (`"true"` or `"false"`).
...
| Tab                    | Settings                                                                                                                                                                                  |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **General**            | Sections: _Calculator_ (font family, font size, autocomplete), _Appearance_ (opacity, animations), _Behavior_ (line numbers, result panel, line wrap, toast notifications), and _Preview_ |
| **Theme**              | Choose from 27 built-in color themes (+ plugin themes) — applies instantly                                                                                                                |
| **UI Style**           | Choose from 5 visual styles (Default, Liquid Glass, Material 3, Alivated, Neon) — applies instantly                                                                                       |
...
| `[settings]` | `font_size`, `font_family`, `shortcut_overrides`, `opacity`, `line_numbers_enabled`, `result_panel_enabled`, `line_wrap_enabled`, `autocomplete_enabled`, `animations_enabled`, `toast_enabled`, `ui_style`, `theme_manually_set`, `noise`, `context_menu_notes`, `context_menu_folders`, `drag_and_drop`, `confirm_dialog` | Font, shortcut, display, and interaction configuration |
